...
insert_papers_query = (
    "insert into dbo.paper output Inserted.paper_id values (%s, %s, %s, %s, %s, %s, %s)"
)
insert_citations_query = (
    "insert into dbo.citation output Inserted.citing_id values (%s, %s, %s)"
)
...
select_relationships_query = "select * from dbo.relationship where paper_id_1 = (%s) and paper_id_1 = (%s) and relationship = (%s)"
        ...
        if len(cited_by) > 0:
            ...
            for cite in cited_by:
                ...
                if result is not None:
                    ...
                    cursor.execute(
                        insert_citations_query, (target_doi, target_pmid, now)
                    )
                    target_citing_id = cursor.fetchone()["citing_id"]
                    conn.commit()

                # search if target is already registered
                cursor.execute(select_papers_query, (target_doi))
                result = cursor.fetchone()
                conn.commit()
                ...
                if result is not None:
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            target_citedby_count,
                        ),
                    )
                    target_paper_id = cursor.fetchone()["paper_id"]
                    conn.commit()

                # search if the mapping is already registered
                cursor.execute(select_relationships_query, (source_citing_id, target_citing_id, "citing"))
                result = cursor.fetchone()
                ...
                if result is None:
                    ...
                    cursor.execute(
                        ...
                        (source_citing_id, target_citing_id, "citing"),
                    )
                    conn.commit()
                    cursor.execute(
                        insert_relationships_query,
                        (target_citing_id, source_citing_id, "cited"),
                    )
                    conn.commit()
